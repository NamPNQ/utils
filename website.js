...
appendAlert("Total: "+$("input[value=\"Visit\"]").size()+" likes");
var count=0;

function setLike(){
    bien_tam=$("input[value=\"Visit\"]").eq(count).attr("onclick").toString().split("'");
    if(!(/['&']/.test($("div.title a b").eq(count).html()))){
  	$.ajax({
        	type: "POST",
        	url: "wreceive.php",
        	data: "data="+bien_tam[1]+ "---" + user_id + "---0---" + bien_tam[9],  
  		    cache: false,
        	success: function(){
                count++;
			    if(count>=$("input[value=\"Visit\"]").size()){
				    setTimeout(function () {  window.location = "http://likesasap.com/website.php"; }, 5000);
                }
				else{
...
if($("input[value=\"Visit\"]").size()>0){
    
for(i=0;i<$("input[value=\"Visit\"]").size();i++){
